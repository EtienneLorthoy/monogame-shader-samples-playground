...
float4 PS1(float4 Position : POSITION0) : COLOR0
{
	float4 color = Test(Position.xy);
	return color;
}
...
float4 PS2(VertexOut input) : SV_TARGET
{
	// Base color
	float4 baseColor = tex2D(ColorMapSampler, input.TexCoord);

	// Ambient stage
	float4 result = baseColor*AmbientColor*AmbientIntensity;
...
technique Technique1
{
	pass P0
	{
		VertexShader = compile VS_SHADERMODEL VS2();
